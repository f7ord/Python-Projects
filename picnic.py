...
def get_args():
    ...
    parser.add_argument(
        ...
        "--sorted",
        action="store_true",
        help="Sort the items to bring, if present",
    )
    parser.add_argument(
        "-c",
        "--comma",
        action="store_true",
        help="If present, print without the Oxford comma",
    )
    parser.add_argument(
        "--sep",
        metavar="separator",
        default=",",
        help="The character to separate items.",
    )
...
def main():
    ...
    if len(items) == 1:
        ...
        if args.comma:
            ...
            print(
                f"You are bringing {sep.join(items[:-1])}{sep.rstrip()} and {items[-1]}."
            )
